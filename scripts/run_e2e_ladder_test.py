...
def _days_with_resolvable_json(metadata: dict, episodes_dir: Path) -> list[str]:
    """Chronological dates that already have a resolvable episode JSON."""
    days: list[str] = []
    for date in dates_in_metadata(metadata):
        for ep in metadata.get("episodes", []):
            if str(ep.get("date")) != date:
                continue
...
episodes_dir = METADATA_DIR / "episodes"
if METADATA_PATH.exists():
    metadata = json.loads(METADATA_PATH.read_text(encoding="utf-8"))
    log(f"Using existing metadata: {METADATA_PATH}")
else:
    log("Merging episode metadata into working/kaggle_episodes (manifest CSVs only)...")
    metadata = merge_episode_metadata(data_dir=METADATA_DIR)
    save_metadata(metadata, METADATA_PATH)
local_available = _days_with_resolvable_json(metadata, episodes_dir)
log(f"Episode days with resolvable *.json: {local_available or '(none)'}")
...
if len(local_bootstrap_days) < _wanted_days:
    raise SystemExit(
        f"E2E smoke needs {_wanted_days} bootstrap day(s) with episode JSON under "
        f"{episodes_dir} or /kaggle/input/kaggriculture-episodes-*; "
        f"found {len(local_available)}: {local_available}"
    )
