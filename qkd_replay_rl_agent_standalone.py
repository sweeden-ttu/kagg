"""Standalone Replay-Guided Statistical QKD Reinforcement Learning Agent for Kaggle.

Self-contained submission module fusing tournament championship replay traces
(100k+ coins) with online QKD statistical questioning, variance-ranked decision probes,
dynamic weed repair, and town shop demand elasticity market order re-ranking.
"""
...
# Championship 100k+ coin replay route from 104756416.json
...
# ── Standalone QKD Statistical Evaluators ──────────────────────────────────────

def _eval_qkd_probes(obs: Dict[str, Any]) -> Dict[str, float]:
    """Lightweight sub-millisecond evaluation of canonical QKD probes."""
    ...
    # Q-Channel: Days Remaining
    q_days_remaining = max(0.0, (30.0 - day) / 30.0)

    # K-Channel: Opponent Wallet Balance
    opp_money = float(opp_farm.get("money", 0.0) or 0.0)
    k_opp_wallet = math.log1p(max(0.0, opp_money)) / 12.0

    # D-Channel: Subagents' Wallets Balance
    my_money = float(my_farm.get("money", 0.0) or 0.0)
    hands = my_farm.get("hands", []) or []
    subagent_multiplier = 1.0 + (len(hands) * 0.1)
    d_subagents_wallet = (math.log1p(max(0.0, my_money)) / 12.0) * subagent_multiplier

    return {
        "Q_DAYS_REMAINING": q_days_remaining,
        "K_OPP_WALLET_BALANCE": k_opp_wallet,
        "D_SUBAGENTS_WALLET_BALANCE": d_subagents_wallet,
    }


# ── Initial Terminal Configuration & Functional Market Suite ───────────────────
...
def build_initial_terminal_configuration(
    ...
    return {
        ...
        "start_price_of_every_commodity": dict(start_prices),
        "days": total_days,
...
def build_market_functions(
    ...
    def predict_future_price(item: str, inventory_delta: int = 0, horizon_hours: int = 0) -> float:
        ...
        demand_rate = _demand_per_day(obs, config, item) / float(_get(config, "turns_per_day", 24) or 24)
        projected_inv = max(0, cur_inv + inventory_delta - int(demand_rate * horizon_hours))
        return float(_market_price(item, projected_inv))
    ...
    def get_price_elasticity(item: str, cur_inventory: int) -> float:
        p_base = float(_market_price(item, cur_inventory))
        p_plus = float(_market_price(item, cur_inventory + 50))
        return (p_plus - p_base) / 50.0

    def get_demand_absorption_rate(item: str) -> float:
        return float(_demand_per_day(obs, config, item))

    def compute_optimal_sell_batch(item: str, cur_inventory: int) -> int:
        demand_day = max(1.0, _demand_per_day(obs, config, item))
        return int(min(250, max(10, demand_day * 0.5)))

    def predict_spot_price(item: str) -> float:
        cur_inv = int(_get(inventory, item, 10000) or 10000)
        return float(_market_price(item, cur_inv))
...
class StandaloneQKDReplayRLAgent:
    """Zero-dependency tournament-ready Replay-Guided QKD Reinforcement Learning Agent."""

    def __init__(self, encoded_route: str = _CHAMPION_REPLAY_ROUTE) -> None:
        self.weed_state: Dict[int, Dict[str, Any]] = {0: {}, 1: {}}
        self.actions = self._load_actions(encoded_route)
    ...
    def Att(
        self,
        obs: Dict[str, Any],
        initial_terminal_configuration: Dict[str, Any],
        ...
        qkd = _eval_qkd_probes(obs)
        day = float(_get(obs, "day", 0) or 0)
        days_rem = qkd.get("Q_DAYS_REMAINING", 1.0) * 30.0
        if days_rem <= 2.0 or day >= 28:
            private = _get(obs, "private", {}) or {}
            shed = _get(private, "shed", {}) or {}
            market_orders = list(repaired_action.get("market") or [])
            for item, qty in shed.items():
                if qty and int(qty) > 0 and item in _MARKET_PARAMS:
                    market_orders.append(["SELL", item, int(qty)])
            repaired_action["market"] = market_orders

        # Check opponent gap
        lead_gap_fn = opponent_functions.get("compute_adversarial_lead_gap")
        if callable(lead_gap_fn) and lead_gap_fn() < 5000.0:
            private = _get(obs, "private", {}) or {}
            shed = _get(private, "shed", {}) or {}
            mkt = list(repaired_action.get("market") or [])
...
_GLOBAL_STANDALONE = StandaloneQKDReplayRLAgent()


def agent(obs: Dict[str, Any], configuration: Any = None) -> Dict[str, Any]:
    return _GLOBAL_STANDALONE.act(obs, configuration)
