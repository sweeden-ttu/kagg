...
# Determined dual-architecture sync (prime facts vs even-hour exploration → day-29 zip).
DUAL_ARCH_SYNC_PLAN: Dict[str, Any] = {
    "flops": {
    ...
    "submission": {
        "zip_max_mb": 90,
        "model_ceiling_mb": 100,
        "patch_buffer_mb": 10,
        "day": 29,
        "hours": [0, 1, 2, 3, 4],
        "order": [
            "freeze_deterministic_code",
            "shrink_probabilistic_weights",
            "build_submission_archive",
            "assert_passed_90mb",
        ],
    },
    "authority_on_conflict": "determined_facts_override_probable_summaries",
    "merge": {
        "path": "artifacts/shared_state.jsonl",
        "prime_writer": "antigravity",
        "even_writer": "cursor",
        "on_withhold": "continue_from_last_committed_row",
    },
    "dialogue": {
        "no_cross_agent_questions_until": "end of day 29 / beginning of day 30",
        "posture_heuristic": "trailing_question_mark_means_questioning_else_reasoning",
    },
}
...
class ReasoningAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.dual_arch_sync: Optional[Dict[str, Any]] = None
        self._submission_sync_armed: bool = False
    ...
    def reset(self) -> None:
        ...
        self.dual_arch_sync = None
        self._submission_sync_armed = False
    ...
    def commit_dual_arch_sync(self) -> Dict[str, Any]:
        """Lock 90 MB / 42 FLOP dual-architecture sync for the remainder of the game.

        Spoken as determined fact (no trailing '?'). Does not reopen Q&A before day 29.
        """
        if self.dual_arch_sync is not None:
            return self.dual_arch_sync
        plan = dict(DUAL_ARCH_SYNC_PLAN)
        statement = (
            "Dual-architecture sync locked: FLOPs are per-agent ≤42 and non-overlapping "
            "(prime hours vs even hours). One shared submission zip ≤90 MB with 10 MB "
            "patch under the 100 MB model ceiling. Day 29 hours 0-4 are the zip window. "
            "Determined facts override probable summaries on conflict. "
            "Cross-agent inquiries remain suspended until end of day 29."
        )
        assert not ends_with_question(statement)
        self._last_utterance = statement
        self.dual_arch_sync = plan
        self._submission_sync_armed = True
        self._questions_suspended_until_day29 = True
        write_antigravity_delta("dual_arch_sync.json", {"plan": plan, "statement": statement})
        append_shared_state(
            SharedTurnState(
                day=0,
                hour=0,
                seat=0,
                bank=float(AGENT1_POST_CHARITY_BANK),
                writer="antigravity",
                trust_fellowship=True,
                ends_with_question=False,
                posture="reasoning",
                note="Dual-arch 90MB/42FLOP sync committed for remainder of game",
                payload={"plan": plan},
            )
        )
        self.day_question_log.append(
        ...
        self.action_audit.append(
            {
                ...
                "hour": -1,
                "acted": True,
                "op": "COMMIT_DUAL_ARCH_SYNC",
                "ends_with_question": False,
            }
        )
        return plan

    def run_submission_zip_turn(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        """Day 29 hours 0-4: conserve flops; farm PASS; mark packaging mode."""
        day = int(obs.get("day", 0) or 0)
        hour = int(obs.get("hour", 0) or 0)
        if not in_submission_zip_window(day, hour):
            return dict(PASS_ACTION)
        if not self.turn_budget.consume(1, label="submission_zip_window"):
        ...
        self.action_audit.append(
            {
                ...
                "acted": True,
                "op": "SUBMISSION_ZIP",
                "flops_remaining": self.turn_budget.remaining(),
                "sync_armed": self._submission_sync_armed,
            }
        )
        return dict(PASS_ACTION)
    ...
    def act(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if in_submission_zip_window(day, hour):
            action = self.run_submission_zip_turn(obs)
            self.turn_budget.force_observable()
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dual_arch_sync": self.dual_arch_sync,
            "submission_sync_armed": self._submission_sync_armed,
