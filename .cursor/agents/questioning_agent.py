...
class QuestioningAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self._elon_musk_stake = int(elon_musk_stake)
        # Schmidt's response (888) is not Elon's stake (menu 888/1667/3887; default 1667).
        self.schmidt: EricSchmidtSubagent = assign_eric_schmidt(
            self._schmidt_stake, bank=self.post_charity_bank, trusted=True
        )
        self.elon_musk: ElonMuskSubagent = assign_elon_musk(
            self._elon_musk_stake, bank=self.post_charity_bank
        )
        self.cursor_bootstrap: Optional[Dict[str, Any]] = None
        self._last_utterance: str = ""
        self.dual_arch_sync_ack: Optional[Dict[str, Any]] = None
    ...
    def reset(self) -> None:
        ...
        self.elon_musk = assign_elon_musk(
            self._elon_musk_stake,
            bank=self.post_charity_bank,
        )
        self.cursor_bootstrap = None
        self._last_utterance = ""
        self.dual_arch_sync_ack = None

    def open_cursor_shared_protocol(self) -> Dict[str, Any]:
        """First-turn Cursor move: Schmidt weight + Elon Musk stake; end with a question."""
        self.cursor_bootstrap = bootstrap_cursor_first_turn(
            schmidt_stake=self.schmidt.stake,
            elon_musk_stake=self.elon_musk.stake,
            bank=self.post_charity_bank,
        )
        ...
        self.action_audit.append(
            {
                ...
                "elon_musk": self.elon_musk.snapshot(),
    ...
    def accept_dual_arch_sync(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        """Even-hour / Cursor seat: acknowledge Agent1's 90MB/42FLOP sync plan.

        Must end with a question so posture stays questioning for the remainder.
        """
        q = (
            "Confirmed: I will keep even-hour exploration ≤42 FLOPs and shrink "
            "probabilistic weights for the shared ≤90 MB day-29 zip — "
            "will you freeze deterministic code before I cut model size?"
        )
        assert ends_with_question(q)
        ack = {
            "accepted": True,
            "writer": "cursor",
            "seat": "agent2_questioning",
            "plan": plan,
            "schmidt_stake": self.schmidt.stake,
            "elon_musk_stake": self.elon_musk.stake,
            "closing_question": q,
        ...
        self.dual_arch_sync_ack = ack
        self._last_utterance = q
        write_cursor_delta("dual_arch_sync_ack.json", ack)
        append_shared_state(
            SharedTurnState(
                day=0,
                hour=0,
                seat=1,
                bank=float(self.post_charity_bank),
                writer="cursor",
                trust_fellowship=self._agent1_trusted,
                schmidt_stake=self.schmidt.stake,
                elon_musk_stake=self.elon_musk.stake,
                ends_with_question=True,
                posture="questioning",
                note="Cursor acknowledged dual-arch sync; ends with question",
                payload={"closing_question": q, "plan_keys": sorted(plan.keys())},
            )
        )
        self.day_question_log.append(
            {
                ...
                "kind": "question",
                "q": q,
                "a": "(awaiting Agent1 freeze of deterministic code)",
                "ends_with_question": True,
                "posture": "questioning",
            }
        )
    ...
    def metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dual_arch_sync_ack": self.dual_arch_sync_ack,
            "dialogue_posture": self.dialogue_posture(),
            "schmidt": self.schmidt.snapshot() if self.schmidt else None,
            "elon_musk": self.elon_musk.snapshot() if self.elon_musk else None,
